...
const HomePage: React.FC = () => {
  ...
  const formattedDate = dayjs(data?.forecast.forecastday[0].date).format("ddd, DD MMMM");
    ...
    <ScrollView className="flex-1">
      ...
      <View className="h-full overflow-scroll bg-background p-4 pt-14">
        ...
        <View className="items-center justify-center">
          <Image
            className="mt-16"
            source={data?.current?.condition?.text ? getWeatherIcon(data.current.condition.text) : null}
       ...
       <View className="mt-6 flex-row justify-between items-center w-full px-12">
        ...
        <View className="flex-row justify-between items-center w-full px-12">
          <Text className="text-primary text-xl">{data?.current.wind_kph} km/h</Text>
          <Text className="text-primary text-xl">{data?.current.humidity}%</Text>
          <Text className="text-primary text-xl">{data?.current.pressure_mb} hPa</Text>
        </View>
        ...
        />
        <TouchableOpacity onPress={handleSearch} className="bg-blue-500 p-2 rounded">
